...
def select_value_and_count_of_most_prolific_species
	"SELECT species, COUNT(characters.species) AS species_occurrence FROM characters GROUP BY characters.species ORDER BY species_occurrence DESC LIMIT 1;"
end
...
def select_character_names_and_number_of_books_they_are_in
	"SELECT characters.name, COUNT(character_books.book_id) AS books_occurrence FROM characters JOIN character_books ON characters.id = character_books.character_id GROUP BY characters.name ORDER BY books_occurrence DESC "
end
